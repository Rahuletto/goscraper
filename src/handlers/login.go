...
// LookupResponse is returned by Lookup and CaptchaVerify endpoints
...
// PasswordResponse is returned by password verification endpoint
...
// LoginResponse for backwards compatibility
...
// Lookup performs email verification (Step 1 of login)
// Returns identifier + digest if successful, or captcha requirement if needed
func (lf *LoginFetcher) Lookup(username string) (*LookupResponse, error) {
	...
	if errors, ok := data["errors"].([]interface{}); ok && len(errors) > 0 {
		...
		if statusCode == 400 {
			...
			if strings.Contains(message, "HIP") || strings.Contains(errMsg, "HIP") {
				if cdigestVal, ok := data["cdigest"].(string); ok && cdigestVal != "" {
					// Fetch captcha image
	...
	if !strings.Contains(message, "User exists") {
		// Check if captcha is required (non-error case)
		if strings.Contains(message, "HIP") {
			if cdigestVal, ok := data["cdigest"].(string); ok && cdigestVal != "" {
				captchaImage, err := lf.FetchCaptcha(cdigestVal)
				if err != nil {
					return &LookupResponse{
	...
	// Success - extract identifier and digest
...
// CaptchaVerify submits captcha answer and returns new lookup data
func (lf *LoginFetcher) CaptchaVerify(username, cdigest, captcha string) (*LookupResponse, error) {
	user := strings.Replace(username, "@srmist.edu.in", "", 1)
	url := fmt.Sprintf("https://academia.srmist.edu.in/accounts/p/40-10002227248/signin/v2/lookup/%s@srmist.edu.in", user)
	...
	// Include captcha and cdigest in the request body
	body := fmt.Sprintf("mode=primary&cli_time=%d&orgtype=40&service_language=en&serviceurl=https%%3A%%2F%%2Facademia.srmist.edu.in%%2Fportal%%2Facademia-academic-services%%2FredirectFromLogin&captcha=%s&cdigest=%s", cli_time, captcha, cdigest)
	req.SetBody([]byte(body))
	...
	if errors, ok := data["errors"].([]interface{}); ok && len(errors) > 0 {
		...
		// Captcha might be wrong, check if new captcha is required
		if statusCode == 400 {
			if strings.Contains(message, "HIP") || strings.Contains(errMsg, "HIP") || strings.Contains(message, "CAPTCHA") || strings.Contains(errMsg, "CAPTCHA") {
				if cdigestVal, ok := data["cdigest"].(string); ok && cdigestVal != "" {
					captchaImage, err := lf.FetchCaptcha(cdigestVal)
					if err != nil {
						return &LookupResponse{
	...
	if !strings.Contains(message, "User exists") {
		// Check if captcha required again
		if strings.Contains(message, "HIP") || strings.Contains(message, "CAPTCHA") {
			if cdigestVal, ok := data["cdigest"].(string); ok && cdigestVal != "" {
				captchaImage, err := lf.FetchCaptcha(cdigestVal)
				if err != nil {
					return &LookupResponse{
...
// VerifyPassword performs password verification (Step 2 of login)
func (lf *LoginFetcher) VerifyPassword(identifier, digest, password string) (*PasswordResponse, error) {
	...
	url := fmt.Sprintf(
		"https://academia.srmist.edu.in/accounts/p/40-10002227248/signin/v2/primary/%s/password?digest=%s&cli_time=%d&servicename=ZohoCreator&service_language=en&serviceurl=https://academia.srmist.edu.in/portal/academia-academic-services/redirectFromLogin",
		identifier, digest, time.Now().UnixMilli(),
	)
	...
	// Check for captcha requirement during password verification
	if captchaData, ok := data["captcha"].(map[string]interface{}); ok {
		if required, ok := captchaData["required"].(bool); ok && required {
			message := ""
			if msg, ok := data["message"].(string); ok {
				message = msg
			}
	...
	// Check for password auth result
	var code interface{}
	var message string

	if passwordAuthVal, ok := data["passwordauth"].(map[string]interface{}); ok {
		code = passwordAuthVal["code"]
	}
	...
	if strings.Contains(strings.ToLower(message), "invalid") || strings.Contains(cookies, "undefined") || cookies == "" {
		return &PasswordResponse{
			Authenticated: false,
